...
impl ResponseError for PublishError {
    fn error_response(&self) -> HttpResponse {
        match self {
            PublishError::UnexpectedError(_) => HttpResponse::new(StatusCode::UNAUTHORIZED),
...
pub async fn publish_newsletter(
    ...
    for subscriber in subscribers {
        // The compiler enforces us to handle both cases
        match subscriber {
            Ok(subscriber) => {
                email_client
                    .send_email(
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let base64encoded_seg = header_value
        .strip_prefix("Basic ")
        .context("The 'Authorization' scheme was not 'Basic'")?;

    let decoded_bytes = base64::decode_config(base64encoded_seg, base64::STANDARD)
...
async fn validate_credentials(
    ...
    // So, even if the default password ends up matching with the provided pass
