...
pub async fn subscribe(
    ...
    let subscriber_id = insert_subscriber(&mut transaction, &new_subscriber)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Failed to insert a new subscriber in the database".into(),))?;
    let subscription_token = generate_subscription_token();

    store_token(&mut transaction, subscriber_id, &subscription_token)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Failed to store the confirmation token for a new subscriber".into(),))?;

    transaction
        .commit()
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Failed to commit SQL transaction to store a new subscriber".into(),))?;
